    ...
    computed: {
        ...
        object_track_segments: function () {
            ...
            this.indexed_person_tracks.forEach(object_tracks => {
                ...
                if (!added) segments['person'].segments.push([object_tracks.start_time, object_tracks.end_time])
            })
            return segments
        }
    },
...
function draw_person_landmarks(landmarks, ctx) {
    ...
    for (const [key, point] of Object.entries(landmarks)) {
        if (point) {
            ctx.fillRect(point.x - landmark_point_half_width, point.y - landmark_point_half_width, landmark_point_width, landmark_point_width)
        }
    }
}
...
class Person_Track {
    constructor(json_data, video_height, video_width) {
        ...
        track.timestamped_objects.forEach(frame => {
            this.frames.push(new Person_Frame(frame, video_height, video_width))
        })
    }
    ...
    most_recent_real_bounding_box(seconds) {
        for (let index = 0; index < this.frames.length; index++) {
            if (this.frames[index].time_offset > seconds) {
                if (index > 0) return this.frames[index - 1].box; else return null
            }
        }
        return null
    }
    most_recent_real_landmarks(seconds) {
        for (let index = 0; index < this.frames.length; index++) {
            if (this.frames[index].time_offset > seconds) {
                if (index > 0) return this.frames[index - 1].landmarks; else return null
            }
        }
        return null
    }
        ...
        for (let index = 0; index < this.frames.length; index++) {
            if (this.frames[index].time_offset > seconds) {
                if (index > 0) {
                    ...
                    const start_box = this.frames[index - 1], end_box = this.frames[index]
    ...
    current_person_landmarks(seconds, interpolate = true) {
        if (interpolate) return this.most_recent_interpolated_landmarks(seconds)
        else return this.most_recent_real_landmarks(seconds)
    }
}